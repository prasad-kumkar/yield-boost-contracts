...
library BorrowLogic {
  ...
  function executeBorrow(
    ...
    params.borrowFee = params.amount * params.borrowFee / 1e18;
    if(params.borrowFee > 0){
      params.amount = params.amount + params.borrowFee;
    }
    ...
    if (params.releaseUnderlying) {
      IAToken(reserveCache.aTokenAddress).transferUnderlyingTo(params.user, params.amount - params.borrowFee);
      if(params.borrowFee > 0){
        IAToken(reserveCache.aTokenAddress).transferUnderlyingTo(
          IAToken(reserveCache.aTokenAddress).RESERVE_TREASURY_ADDRESS(), 
          params.borrowFee
        );
      }
    }
