...
contract Pool is VersionedInitializable, PoolStorage, IPool {
    ...
    uint256 public constant POOL_REVISION = 2;
    IPoolAddressesProvider public immutable ADDRESSES_PROVIDER;

    // V2 functions
    mapping (address => uint) public supplyFees;
    mapping (address => uint) public borrowFees;
    ...
    function supplyWithPermit(
        ...
        IERC20WithPermit(params.asset).permit(
            msg.sender,
            address(this),
            params.amount,
            params.deadline,
        ...
        SupplyLogic.executeSupply(
            ...
            DataTypes.ExecuteSupplyParams({
                ...
                supplyFee: supplyFees[params.asset]
            })
        );
    }
    ...
    event UpdateAssetFees(address asset, uint256 supplyFee, uint256 borrowFee);

    function updateAssetFees(
        address[] memory assets,
        uint256[] memory supplyFee,
        uint256[] memory borrowFee
    ) external virtual onlyPoolAdmin {
        require(assets.length == supplyFee.length, "supplyFee length mismatch");
        require(assets.length == borrowFee.length, "borrowFee length mismatch");
        for (uint i = 0; i < assets.length; i++) {
            supplyFees[assets[i]] = supplyFee[i];
            borrowFees[assets[i]] = borrowFee[i];
            emit UpdateAssetFees(assets[i], supplyFee[i], borrowFee[i]);
        }
    }
}